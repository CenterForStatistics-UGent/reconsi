...
#'
#' @param densPerm A matrix with B rows of density estimates of the B
...
#' @param zIndObs An indicator vector of the observed z-values
#'    in the support of the kernel
...
calcWeights = function(densPerm, weightStrat, zIndObs, fdr, zIndR){
  weights = switch(weightStrat,
                   ...
                   "LHw" = exp(stabExp(colSums(log(densPerm[zIndObs,])*
                                                   fdr[zIndObs]))),
