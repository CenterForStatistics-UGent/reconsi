...
estP0 = function(zValObs, nullDensCum, zSeq, z0quantRange, smooth.df){
  pi0 = sapply(z0quantRange, function(z0Quant) {
    ...
    centralBorders = zSeq[c(which.max(nullDensCum > z0Quant[1]), which.max(nullDensCum > z0Quant[2]))]
    mean(zValObs <= centralBorders[2] & zValObs >= centralBorders[1])/diff(z0Quant)
  })
