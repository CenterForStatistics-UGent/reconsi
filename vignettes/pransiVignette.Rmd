...
```{r kruskal}
fdrResKruskal = rransi(mat, x, B = 5e1,
test = function(x, y){
 kruskal.test(y~x)$statistic}, zValues = FALSE)
```

Alternatively, the same permutations may be used to determine the marginal null distributions as to estimate the random null distribution, by setting the "permZvals" flag to true.
...
```{r Vandeputte}
testVanDePutte = testDAA(Vandeputte, "Health.status", "absCountFrozen", B = 5e1)
```
